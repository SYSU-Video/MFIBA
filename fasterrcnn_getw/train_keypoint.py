...
from model.faster_rcnn import fasterrcnn_resnet50_fpn_ljl, fasterrcnn_mobilenet_v3_large_320_fpn, fasterrcnn_mobilenet_v3_large_fpn
from model.keypoint_rcnn import keypointrcnn_resnet50_fpn
...
def main(args):
    ...
    if args.model == 'resnet50':
        model = fasterrcnn_resnet50_fpn_ljl(num_classes=num_classes, pretrained=args.pretrained, trainable_backbone_layers=0) # 默认backbone为fasterrcnn_resnet50_fpn
    elif args.model == 'mobilenet_v3':
        model = fasterrcnn_mobilenet_v3_large_fpn(num_classes=num_classes, pretrained=args.pretrained)
    elif args.model == 'mobilenet_v3_320':
        model = fasterrcnn_mobilenet_v3_large_320_fpn(num_classes=num_classes, pretrained=args.pretrained)
    elif args.model == 'keypoint_rcnn':
        model = keypointrcnn_resnet50_fpn(num_classes=num_classes, pretrained=args.pretrained)
    ...
    if args.test_only:
        evaluate(model, data_loader_test, device=device, lmbda=args.lmbda_for_update)
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr-steps', default=[3, 4], nargs='+', type=int, help='decrease lr every step-size epochs')
    ...
    # parser.add_argument('--noise_layer', default="-1", type=str, help='noise_layer')
    # parser.add_argument('--noise', default="uni", type=str, help='noise_type')
    # parser.add_argument('--noise_l', default=0, type=float, help='noise_lamda')
    # parser.add_argument('--mask_type', default="none", type=str, help='mask_type')
    # parser.add_argument('--mask_roi', default=1, type=int, help='mask_roi')
    parser.add_argument(
        "--pretrained",
        dest="pretrained",
    ...
    parser.add_argument('--lmbda_for_update', default=500, type=int, help='lmbda')
